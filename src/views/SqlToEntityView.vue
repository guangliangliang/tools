  ...
  <div class="sql-converter">
    ...
    <div class="toolbar">
      <div class="toolbar-group">
        ...
        <div class="db-selector">
          <label>语言</label>
          <select v-model="language" @change="generate">
    ...
    <div class="config-bar" v-if="language === Language.JAVA">
      <label><input type="checkbox" v-model="options.useLombok" @change="generate"> Lombok</label>
      <label><input type="checkbox" v-model="options.useJpa" @change="generate"> JPA</label>
    </div>
    ...
    <div class="editor-container">
      ...
      <div class="editor-panel">
        <div class="panel-header">
          <div class="header-left">
            <span class="header-icon">📤</span>
            <span>生成的 {{ getLanguageName(language) }} 代码</span>
          </div>
...
import type { GeneratorOptions } from '../utils/codeGenerator'
import { 
  Language, 
  generateCode 
...
const outputLength = computed(() => outputCode.value.length)
...
const generate = () => {
  ...
  try {
    ...
    const result = generateCode(parsed, opts)
    outputCode.value = result
    updateOutputEditor(result)
    message.value = '生成成功！'
  } catch (e) {
    message.value = `生成错误：${(e as Error).message}`
    outputCode.value = ''
    updateOutputEditor('')
  }
}
  ...
  if (outputEditor) {
    ...
    if (model) {
      ...
      switch (language.value) {
        case Language.PYTHON: lang = 'python'; break
        case Language.CSHARP: lang = 'csharp'; break
        case Language.GO: lang = 'go'; break
      }
