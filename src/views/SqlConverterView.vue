  ...
  <div class="sql-converter">
    <div class="page-header">
      <div class="header-content">
        ...
        <p class="page-desc">支持 MySQL、PostgreSQL、Oracle 三大主流数据库之间的 SQL 语法转换</p>
      </div>
    </div>
    ...
    <div class="toolbar">
      <div class="toolbar-group">
        <div class="db-selector">
          <label>源数据库</label>
          <select v-model="sourceDb" @change="loadSample">
        ...
        <div class="db-selector">
          <label>目标数据库</label>
          <select v-model="targetDb" @change="convert">
            <option :value="DatabaseType.MYSQL">MySQL</option>
            <option :value="DatabaseType.POSTGRESQL">PostgreSQL</option>
            <option :value="DatabaseType.ORACLE">Oracle</option>
          </select>
      ...
      <div class="toolbar-group">
        ...
        <button @click="loadSample" class="btn btn-warning">
          <span class="btn-icon">📥</span>
    ...
    <div class="editor-container">
      <div class="editor-panel">
        <div class="panel-header">
          <div class="header-left">
            ...
            <span>{{ getDatabaseName(sourceDb) }} SQL</span>
          </div>
      ...
      <div class="editor-panel">
        <div class="panel-header">
          <div class="header-left">
            <span class="header-icon">📤</span>
            <span>{{ getDatabaseName(targetDb) }} SQL</span>
          </div>
...
import { DatabaseType, convertSql, getDatabaseName, getSampleSql } from '../utils/sqlConverter'

const sourceDb = ref<DatabaseType>(DatabaseType.MYSQL)
const targetDb = ref<DatabaseType>(DatabaseType.POSTGRESQL)
const inputSql = ref('')
const outputSql = ref('')
const message = ref('')
...
const inputLength = computed(() => inputSql.value.length)
const outputLength = computed(() => outputSql.value.length)

const messageType = computed(() => {
  if (message.value.includes('错误')) return 'error'
  return 'success'
})
...
const convert = () => {
  const value = inputSql.value.trim()
  if (!value) {
    message.value = '请输入 SQL 内容'
    return
  }

  try {
    const result = convertSql(value, sourceDb.value, targetDb.value)
    outputSql.value = result
    updateOutputEditor(result)
    message.value = '转换成功！'
  } catch (e) {
    message.value = `转换错误：${(e as Error).message}`
    outputSql.value = ''
    updateOutputEditor('')
  }
}
...
const loadSample = () => {
  const sample = getSampleSql(sourceDb.value)
  inputSql.value = sample
  if (inputEditor) {
    inputEditor.setValue(sample)
  }
  message.value = '已加载示例 SQL'
  convert()
}
...
const initEditors = () => {
  ...
  if (inputEditorRef.value) {
    ...
    inputEditor.onDidChangeModelContent(() => {
      if (inputEditor) {
        inputSql.value = inputEditor.getValue()
      }
    })
  }
